...
function addImageToPdfFitPage(pdf, imgData, imgType, sourceWidth, sourceHeight) {
    const pageWidth = pdf.internal.pageSize.getWidth();
    const pageHeight = pdf.internal.pageSize.getHeight();

    const footerText = (arguments.length >= 6 ? arguments[5] : "") || "";
    const footerHeight = footerText ? 12 : 0;
    ...
    if (footerText) {
        ...
        const textX = Math.max(0, (pageWidth - textWidth) / 2);
        pdf.text(footerText, textX, pageHeight - 4);
    }
}
...
function handleResolutionChange() {
    ...
    overrideDepthPixelArray = new Array(targetResolution[0] * targetResolution[1] * 4).fill(null);
    resetUndoRedoHistory();
    populateInstructionSectionSizeOptions();
    document.getElementById("width-text").title = `${(targetResolution[0] * PIXEL_WIDTH_CM).toFixed(1)} cm, ${(
        targetResolution[0] *
        PIXEL_WIDTH_CM *
        INCHES_IN_CM
...
function populateInstructionSectionSizeOptions() {
    const select = document.getElementById("instruction-section-size-select");
    if (!select) {
        return;
    }

    const prev = Number(select.value || PLATE_WIDTH);
    const width = Number(targetResolution[0]);
    const height = Number(targetResolution[1]);

    // Only allow sizes that divide both width and height exactly so sections are consistent.
    ...
    const fallback = sizes.includes(PLATE_WIDTH) ? PLATE_WIDTH : sizes[sizes.length - 1] || PLATE_WIDTH;
    select.value = String(sizes.includes(prev) ? prev : fallback);
}
...
async function generateInstructions() {
    ...
    disableInteraction();
    runStep4(async () => {
        const footerText = (document.getElementById("instruction-footer-text")?.value || "").trim();

        const minimapMode = (document.getElementById("instruction-minimap-mode")?.value || "image").trim();
        const includeGuestInstructions = !!document.getElementById("instruction-guest-instructions-check")?.checked;

        const selectedSectionSize = Number(
            document.getElementById("instruction-section-size-select")?.value || PLATE_WIDTH
        );
        const sectionSize = Number.isFinite(selectedSectionSize) && selectedSectionSize > 0 ? selectedSectionSize : PLATE_WIDTH;

        if (targetResolution[0] % sectionSize !== 0 || targetResolution[1] % sectionSize !== 0) {
            enableInteraction();
            throw new Error(
                `Invalid section size ${sectionSize}. It must evenly divide the resolution (${targetResolution[0]}x${targetResolution[1]}).`
            );
        }
        ...
        for (var i = 0; i < totalPlates; i++) {
            ...
            const subPixelArray = getSubPixelArray(resultImage, i, targetResolution[0], sectionSize);

            const row = Math.floor((i * sectionSize) / targetResolution[0]);
            const col = i % (targetResolution[0] / sectionSize);
